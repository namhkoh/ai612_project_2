...
import random
import abc
from src.types import Tool, AgentRunResult
...
class Agent(abc.ABC):
    ...
    @abc.abstractmethod
    def run(self, env: 'Env', task_index: Optional[int] = None, max_num_steps: int = 30) -> AgentRunResult:
        """Run the agent on the given environment.
        
        Args:
            env: The environment to run the agent on
            task_index: Optional task index to run
            max_num_steps: Maximum number of steps to run
            
        Returns:
            AgentRunResult containing the results of the run
        """
